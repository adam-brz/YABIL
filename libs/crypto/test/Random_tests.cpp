...
{
public:
    RandomEngine engine{7};
};

TEST_F(UtilsRandom_tests, canGenerateRandomNumber)
{
    ASSERT_NO_THROW({ [[maybe_unused]] const BigInt num = engine.random_bigint(32); });
}

TEST_F(UtilsRandom_tests, randomPrimeMustHaveMoreThan2bits)
{
    ASSERT_THROW(engine.random_prime(1), std::invalid_argument);
}

TEST_F(UtilsRandom_tests, canGenerateRandomNumberWithSetTopAndBottomBit)
{
    const BigInt num = engine.random_bigint(64, true, true);
...
TEST_F(UtilsRandom_tests, canGenerateRandomNumberFromGivenRange)
{
    ...
    {
        const BigInt num = engine.random_bigint(BigInt(100), BigInt(200));
...
TEST_F(UtilsRandom_tests, canGenerateRandomNumberFromGivenRange_1)
{
    ...
    const BigInt num = engine.random_bigint(min, max);
    ASSERT_TRUE(num >= min && num <= max);
}

TEST_F(UtilsRandom_tests, canGenerateLargeRandomPrimeNumber)
{
    ASSERT_NO_THROW({ [[maybe_unused]] const BigInt num = engine.random_prime(128); });
#ifdef NDEBUG
    ASSERT_NO_THROW({ [[maybe_unused]] const BigInt num = engine.random_prime(1024); });
#endif
}
