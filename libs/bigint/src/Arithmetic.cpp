                                                                                 ...
                                                                                 std::span<bigint_base_t const> b)
{
    ...
    while (m > n)
    {
        const auto [q, r] = recursive_div({A.cbegin() + (m - n), A.cend()}, b);
    ...
    const auto [q, r] = recursive_div(A, b);
                                                                                ...
                                                                                std::span<bigint_base_t const> b)
{
    ...
    auto [Q1, R1] = recursive_div(a.subspan(2UL * k), B1);
    const auto R1_shifted = BigInt(R1) << (digit_size_bits * 2 * k);
    auto A_prim = BigInt(plain_add(R1_shifted.raw_data(), a.subspan(0, 2UL * k))) -
    ...
    auto [Q0, R0] = recursive_div(A_prim_view.subspan(k), B1);

    const auto R0_shifted = BigInt(R0) << (digit_size_bits * k);
    auto A_bis = BigInt(plain_add(R0_shifted.raw_data(), A_prim_view.subspan(0, k))) - BigInt(karatsuba_mul(Q0, B0));
