...
TEST_F(BigIntShiftOperator_tests, shiftEntireNumber)
{
    ...
    EXPECT_EQ(expected, (big_int << (sizeof(bigint_base_t) * 8)).raw_data());
}
...
TEST_F(BigIntShiftOperator_tests, shiftHalfOfEntireNumber)
{
    ...
    EXPECT_EQ(expected, (big_int << (sizeof(bigint_base_t) * 8 / 2)).raw_data());
}
...
{
    const BigInt big_int(std::numeric_limits<bigint_base_t>::max());
    EXPECT_EQ(0, (big_int >> (sizeof(bigint_base_t) * 8)).to_int());
}
