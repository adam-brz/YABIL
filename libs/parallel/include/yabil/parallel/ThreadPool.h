...
public:
    ...
    YABIL_PARALLEL_EXPORT explicit ThreadPool(int concurrency = 0);

    /// @brief Stops all threads from the pool and waits for their end.
    YABIL_PARALLEL_EXPORT virtual ~ThreadPool();

    YABIL_PARALLEL_EXPORT ThreadPool(const ThreadPool &) = delete;
    YABIL_PARALLEL_EXPORT ThreadPool(ThreadPool &) = delete;
    ...
    YABIL_PARALLEL_EXPORT unsigned currently_running_tasks_count() const;
    ...
    YABIL_PARALLEL_EXPORT void resize(std::size_t new_size);
