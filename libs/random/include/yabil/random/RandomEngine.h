...
#include <yabil/random/random_export.h>
...
/// @headerfile RandomEngine.h <yabil/random/RandomEngine.h>
class RandomEngine
{
private:
    class Impl;
...
public:
    ...
    YABIL_RANDOM_EXPORT RandomEngine(const std::size_t seed = std::random_device{}());
    ...
    YABIL_RANDOM_EXPORT yabil::bigint::BigInt random_bigint(uint64_t number_of_bits = 64, bool top_two = false,
    ...
    /// @return Pseudo-random \p BigInt
    YABIL_RANDOM_EXPORT yabil::bigint::BigInt random_bigint(const yabil::bigint::BigInt &min,
                                                            const yabil::bigint::BigInt &max);

    /// @brief Generate large pseudo-random prime number.
    /// @param number_of_bits Number of bits for prime number
    /// @return Pseudo-random prime \p BigInt
    YABIL_RANDOM_EXPORT yabil::bigint::BigInt random_prime(uint64_t number_of_bits = 1024);

    /// @brief Generate single random digit using specified distribution.
    /// @param distribution Standard random distribution to use.
    YABIL_RANDOM_EXPORT bigint::bigint_base_t random_digit(
