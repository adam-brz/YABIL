...
{
    ASSERT_EQ(BigInt(0), sqrt(BigInt(0)));
}

TEST_F(MathSqrt_tests, sqrtOfOneIsOne)
{
    ASSERT_EQ(BigInt(1), sqrt(BigInt(1)));
}

TEST_F(MathSqrt_tests, sqrtOfTwoIsOne)
{
    ASSERT_EQ(BigInt(1), sqrt(BigInt(2)));
}

TEST_F(MathSqrt_tests, sqrtAcceptanceTests)
{
    ASSERT_EQ(BigInt(4), sqrt(BigInt(16)));
    ASSERT_EQ(BigInt(128), sqrt(BigInt(16384)));
    ASSERT_EQ(BigInt("1161692"), sqrt(BigInt("1349528751497")));
    ASSERT_EQ(BigInt("3305826693656"), sqrt(BigInt("10928490128490812093812903")));
}
